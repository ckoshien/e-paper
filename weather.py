...
try:
    ...
    for i in range(12):
        ...
        draw.text((10, 65 + i*60), str(i*2), font = jaFont, fill = 0)
        draw.text((80, 65 + i*60), convertWeatherCode(data['hourly']['weathercode'][i*2+24]), font = jaFont, fill = 0)
        draw.text((180, 65 + i*60), str(data['hourly']['temperature_2m'][i*2+24]) + u'℃', font = jaFont, fill = 0)
        draw.text((300, 65 + i*60), convertWeatherCode(data['hourly']['weathercode'][i*2+48]), font = jaFont, fill = 0)
        draw.text((400, 65 + i*60), str(data['hourly']['temperature_2m'][i*2+48])+ u'℃', font = jaFont, fill = 0)
